...
private:
    ros::NodeHandle n;
    ros::Subscriber sub = n.subscribe("/joy", 1, &PuckDobot::joyCallback, this);
...
public:
    PuckDobot()
    {
        ...
        do {
            ...
            srv.request.velocityRatio = 100;
            srv.request.accelerationRatio = 200;
            client.call(srv);
        } while (0);
    ...
    void joyCallback(const sensor_msgs::Joy::ConstPtr& msg){
        ...
        ptp_srv.request.ptpMode = 1;
        ptp_srv.request.x = int(joy_x1*100);
        ptp_srv.request.y = int(joy_y1*100);
        ptp_srv.request.z = int(joy_y2*100);
        ptp_srv.request.r = 0;
        client.call(ptp_srv);
	ROS_INFO_STREAM(srv.response.result);
	ROS_INFO("x:%f, y:%f, z:%f", joy_x1*100, joy_y1*100, joy_y2*100);
    }
};
